...
$(function () {
	$('#inline-footer a[href=http://www.pokemon-gl.com/languages/]')
	...
	if ({ en:1, fr:1, it:1, de:1, es:1 }[theme.language]) {
		var gus_src = theme.host_code == 2918 ? 'http://gus.pokemon.com/' + theme.language + '/' : 'http://' + theme.get_com_domain() + '/pgl-363/' + theme.language + '/';
...
theme.bind('initialize', function (event) {
	...
	if (theme.level != theme.NOT_SIGNED_UP) {
		...
		swfobject.embedSWF('http://' + theme.get_swf_host() + '/src/swf/theme/assets/swf/volume.swf', 'top-volume', '56', '12', '9.0.0', null, { color1:'0x575757', color2:'0xA7A7A7' }, { wmode:'transparent', allowScriptAccess:'always' });
	}
	...
	if (theme.level >= theme.NOT_SIGNED_UP) {
		...
		if (data.member.rom_id == '20') {
			$('#inline-footer img').attr({ src:'http://' + theme.get_page_asset_host() + '/src/swf/top-assets/' + theme.language + '/images/footer-white.png' });
		} else if (data.member.rom_id == '21') {
			$('#inline-footer img').attr({ src:'http://' + theme.get_page_asset_host() + '/src/swf/top-assets/' + theme.language + '/images/footer-black.png' });
		} else if (theme.level >= theme.TRIAL) {
			$('#inline-footer img').attr({ src:'http://' + theme.get_page_asset_host() + '/src/swf/top-assets/' + theme.language + '/images/footer-trial.png' });
		} else {
			$('#inline-footer img').attr({ src:'http://' + theme.get_page_asset_host() + '/src/swf/top-assets/' + theme.language + '/images/footer-logout.png' });
		}
		...
		if (theme.level >= theme.TRIAL) {
			...
			$('<img width="36" height="36" class="member-info-avatar"/>')
				.attr({ src:'http://' + theme.get_page_asset_host() + '/src/swf/top-assets/images/avatar/' + data.member.avator_id + '.png', alt:data.member.avator_name })
				.appendTo($('#login-control .member-info'));
			
			$('<div id="login-menu"></div>')
				.append($('<div class="login-menu-profile swap"><a class="sidebar-ui" href="profile/"></a></div>'))
				.append($('<div class="login-menu-logout swap"><a class="sidebar-ui" href="/?p=logout"></a></div>'))
				.appendTo($('#login-control .member-info'));
			if (location.host.indexOf('pokemon-gl') == -1) {
				$('#login-menu .login-menu-logout a').attr({ href:'/index3?p=logout' });
			}
			...
			$('#inline-footer-map area:not(area[href])').attr({ href:'/traffic/' + traffic_type + '_' + traffic_id + '/' });
		} else {
		...
		$('<img/>').attr({ src:login_url.check + '?now=' + new Date().getTime() }).load(function () {
			...
			$('<iframe class="login-frame" border="0" frameborder="0" allowtransparency="true" scrolling="no" marginwidth="0" marginheight="0"></iframe>')
				.attr({ src: 'http://' + theme.get_page_asset_host() + '/src/swf/top-assets/' + theme.language + '/images/login-unavailable.png' })
				.appendTo($('#login-control'));
		});
	...
	if (theme.level <= theme.NOT_SIGNED_UP) {
		$('#gbu a, #pdw a, #safemail a, #customize a').attr({ href:'/introduction/' });
	} else {
	...
	function update_info() {
		theme.get_api('pgl.top.index', function (data) {
			...
			$.each(data.news_list, function (index) {
				var link = $('<a></a>').attr({ href:'/information/?infoto=' + this.news_id })
					.css({ display:'block' })
				...
				if (this.filename_top) {
					$('<img width="58" height="58" align="left" style="margin-right:4px;"/>').attr({ src:'http://' + theme.get_cms_image_host() + '/src/swf/information/assets/' + theme.language + '/img' + this.filename_top }).prependTo($('.title', link));
			...
			], 'http://' + theme.get_page_asset_host() + '/src/swf/top-assets/' + theme.language + '/images/pgl-stats-chart1-label.png');
			...
			], 'http://' + theme.get_page_asset_host() + '/src/swf/top-assets/' + theme.language + '/images/pgl-stats-chart2-label.png');
	...
	window.show_pokemon_info = function (pokedex, pokename) {
		...
		$('#pokemon-window .pokemon-window-image').empty().append($('<img width="92" height="92"/>').attr('src', 'http://' + theme.get_page_asset_host() + '/src/swf/top-assets/images/pokemon/' + theme.get_pokecode(pokedex) + '.png'));
		$('#pokemon-window .pokemon-window-info .pokemon-footstamp').empty().append($('<img/>').attr('src', 'http://' + theme.get_page_asset_host() + '/src/swf/top-assets/images/footprint/' + pokedex3 + '.png'));
	...
	function createMap() {
		...
		for (var i=0; i<4; i++) {
			...
			for (var j=0; j<8; j++) {
				var r = i;
				var c = j % 6;
				var src = 'http://' + theme.get_page_asset_host() + '/src/swf/top-assets/images/map/map_r' + (r + 1) + '_c' + (c + 1) + '.png';
				$('<td></td>')
