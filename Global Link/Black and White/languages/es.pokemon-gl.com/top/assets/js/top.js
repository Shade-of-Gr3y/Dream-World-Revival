...
//          $('#right-buttons .login-menu-logout').attr({ href:'/es.pokemon-gl.com/index3?p=logout' });
//        }

        var trafficId = data.member.world_id * 12345 - 6789;
        var trafficType = this.level == PGL.TRIAL ? 'trial' : 'product';
        $('#inline-footer-map area:not(area[href])').attr({ href:'/es.pokemon-gl.com/traffic/' + trafficType + '_' + trafficId + '/' });
      ...
      validateLogin(loginUrl.check).then(function () {
        ...
        $('<iframe class="login-frame" border="0" frameborder="0" allowtransparency="true" scrolling="no" marginwidth="0" marginheight="0"></iframe>')
        .attr({ src: '/es.pokemon-gl.com/src/swf/top/' + PGL.language + '/images/login-unavailable.png' })
        .appendTo($('#login-control'));
      });
  ...
  showInfoDialog: function (category, title, body, linkOptions, image) {
    ...
    if (image) {
      ...
      $('<img/>').attr({ src:'/es.pokemon-gl.com/src/swf/information/assets/' + PGL.language + '/img' + image }).appendTo(dialogImage);
    }
  ...
  setTimeout(function () {
    $.getJSON('/swf/json/' + (PGL.language == 'ja' ? 'battledata.json' : 'battledata_' + PGL.language + '.json'), { r:new Date().getTime() }).then(function (data) {
      ...
      function showBattleUser(target, user) {
        ...
        target.find('.avatar').empty().append($('<img/>').attr({ src:'/es.pokemon-gl.com/profile/assets/images/avatar/' + user.avator_id + '.png' }));
  ...
  setTimeout(function () {
    self.getApi('pgl.journal.census', {}, function (data) {
      ...
      function showRecord() {
        ...
        $('#report-records .most-slept-pokemon .footprint img').attr({ src:'/es.pokemon-gl.com/report/assets/images/footprint/' + ('00' + mostSlept.pokedex).substr(-3) + '.png' });
