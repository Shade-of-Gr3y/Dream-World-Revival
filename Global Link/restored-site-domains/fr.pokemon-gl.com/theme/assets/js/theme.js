...
(function () {
	...
	$(function () {
		...
		if (mimages.length == 0) {
			$('#pdc-toplink').attr('href', '/fr.pokemon-gl.com/?p=account.pdc.toplink');
		}
		...
		if (_api_returned && _page_loaded) {
			...
			if (theme.pgl_top_init_result) {
				if (theme.level >= theme.TRIAL) {
					...
					$('#footer-menu-row img').attr({ src:'/fr.pokemon-gl.com/src/swf/theme/assets/' + theme.language + '/images/footer-menu-login.png' });
					$('#footer-menu-map area:not(area[href])').attr({ href:'/fr.pokemon-gl.com/traffic/' + traffic_type + '_' + traffic_id + '/' });
				} else {
					$('#footer-menu-map area:not(area[href])').removeAttr('alt');
					
					if (theme.level == theme.NOT_SIGNED_UP) {
						if (!location.href.match(/register|help/)) {
							location.href = '/fr.pokemon-gl.com/register/?prev_url=' + encodeURIComponent(location.href);
							return;
						}
					}
				}
