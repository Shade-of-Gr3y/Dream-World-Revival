...
$(function () {
	$('#inline-footer a[href=/www.pokemon-gl.com/languages/]')
		.attr('href', '/ja.pokemon-gl.com/languages/');
...
theme.bind('initialize', function (event) {
	...
	if (theme.level >= theme.NOT_SIGNED_UP) {
		...
		if (data.member.rom_id == '20') {
			$('#inline-footer img').attr({ src:'/ja.pokemon-gl.com/src/swf/top-assets/' + theme.language + '/images/footer-white.png' });
		} else if (data.member.rom_id == '21') {
			$('#inline-footer img').attr({ src:'/ja.pokemon-gl.com/src/swf/top-assets/' + theme.language + '/images/footer-black.png' });
		} else if (theme.level >= theme.TRIAL) {
			$('#inline-footer img').attr({ src:'/ja.pokemon-gl.com/src/swf/top-assets/' + theme.language + '/images/footer-trial.png' });
		} else {
			$('#inline-footer img').attr({ src:'/ja.pokemon-gl.com/src/swf/top-assets/' + theme.language + '/images/footer-logout.png' });
		}
		...
		if (theme.level >= theme.TRIAL) {
			...
			$('<img width="36" height="36" class="member-info-avatar"/>')
				.attr({ src:'/ja.pokemon-gl.com/src/swf/top-assets/images/avatar/' + data.member.avator_id + '.png', alt:data.member.avator_name })
				.appendTo($('#login-control .member-info'));
			
			$('<div id="login-menu"></div>')
				.append($('<div class="login-menu-profile swap"><a class="sidebar-ui" href="profile/"></a></div>'))
				.append($('<div class="login-menu-logout swap"><a class="sidebar-ui" href="/ja.pokemon-gl.com/?p=logout"></a></div>'))
				.appendTo($('#login-control .member-info'));
			if (location.host.indexOf('pokemon-gl') == -1) {
				$('#login-menu .login-menu-logout a').attr({ href:'/ja.pokemon-gl.com/index3?p=logout' });
			}
			...
			$('#inline-footer-map area:not(area[href])').attr({ href:'/ja.pokemon-gl.com/traffic/' + traffic_type + '_' + traffic_id + '/' });
		} else {
		...
		$('<img/>').attr({ src:login_url.check + '?now=' + new Date().getTime() }).load(function () {
			...
			$('<iframe class="login-frame" border="0" frameborder="0" allowtransparency="true" scrolling="no" marginwidth="0" marginheight="0"></iframe>')
				.attr({ src: '/ja.pokemon-gl.com/src/swf/top-assets/' + theme.language + '/images/login-unavailable.png' })
				.appendTo($('#login-control'));
		});
	...
	if (theme.level <= theme.NOT_SIGNED_UP) {
		$('#gbu a, #pdw a, #safemail a, #customize a').attr({ href:'/ja.pokemon-gl.com/introduction/' });
	} else {
	...
	function update_info() {
		theme.get_api('pgl.top.index', function (data) {
			...
			$.each(data.news_list, function (index) {
				var link = $('<a></a>').attr({ href:'/ja.pokemon-gl.com/information/?infoto=' + this.news_id })
					.css({ display:'block' })
				...
				if (this.filename_top) {
					$('<img width="58" height="58" align="left" style="margin-right:4px;"/>').attr({ src:'/ja.pokemon-gl.com/src/swf/information/assets/' + theme.language + '/img' + this.filename_top }).prependTo($('.title', link));
	...
	window.show_pokemon_info = function (pokedex, pokename) {
		...
		$('#pokemon-window .pokemon-window-image').empty().append($('<img width="92" height="92"/>').attr('src', '/ja.pokemon-gl.com/src/swf/top-assets/images/pokemon/' + theme.get_pokecode(pokedex) + '.png'));
		$('#pokemon-window .pokemon-window-info .pokemon-footstamp').empty().append($('<img/>').attr('src', '/ja.pokemon-gl.com/src/swf/top-assets/images/footprint/' + pokedex3 + '.png'));
	...
	function createMap() {
		...
		for (var i=0; i<4; i++) {
			...
			for (var j=0; j<8; j++) {
				var r = i;
				var c = j % 6;
				var src = '/ja.pokemon-gl.com/src/swf/top-assets/images/map/map_r' + (r + 1) + '_c' + (c + 1) + '.png';
				$('<td></td>')
