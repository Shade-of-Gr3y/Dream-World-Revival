...
$.extend(PGL.prototype, {
      ...
      if (friend) {
        ...
        if (that.isMyProfile) {
          ...
          if (blocking) {
            ...
            $('<a class="mailer-link">mailer</a>').attr({ href:'/it.pokemon-gl.com/mailer/?mailto=' + friend.member_savedata_id }).appendTo(cell);
          } else {
  ...
  makeGbuTab: function(data) {
    ...
    if (data.ranking && data.ranking[0].gsid) {
      $('#gbu-record-link').attr({ href:'/it.pokemon-gl.com/gbu/?rankingto=' + data.ranking[0].gsid }).parent().show();
    } else {
  ...
  showGsidcRegisterDialog: function() {
    ...
    $('#dialog-gsidc-body-1 .dialog-buttons-register').unbind('click').click(function () {
      ...
      that.getApi('pgl.member.profile.rom_check', { gscd: gscd }, function (data) {
        ...
        rom = data.rom_id;
        var src = '/it.pokemon-gl.com/top/assets/' + PGL.language + '/images/rom-image-' + { 20:'w', 21:'b', 22:'w2', 23:'b2' }[rom] + '.png';
  ...
  showRomChangeDialog: function() {
    ...
    this.getApi('pgl.member.profile.gsid_list', {}, function(data) {
      ...
      $.each(that.gsid, function () {
        ...
        $('<img/>').attr({ src:'/it.pokemon-gl.com/profile/assets/' + PGL.language + '/images/' + pkg }).appendTo($(target).find('.package').empty());
        $('<img/>').attr({ src:'/it.pokemon-gl.com/top/assets/' + PGL.language + '/images/' + logo }).appendTo($(target).find('.logo').empty());
