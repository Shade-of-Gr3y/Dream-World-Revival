package bfp.common
{
   ...
   public class textFieldManager
   {
      ...
      public static function loadedCheckId(param1:*) : Boolean
      {
         ...
         {
            if(stringsFileLoaderList[_loc3_].fileId == param1 && stringsFileLoaderList[_loc3_].checkLoaded)
            {
               _loc2_ = true;
               break;
            }
      ...
      public static function setAntiAlias(param1:*, param2:*, param3:*) : void
      {
         ...
         var _loc4_:XML = _format_dictionary[param2];
         if(_loc4_ && _loc4_.@font.toString() != "")
         {
      ...
      public static function createTextFormatTag(param1:String) : Object
      {
         ...
         if(_loc3_)
         {
            ...
            {
               _loc8_ = _fonts[_loc3_.@font.toString()];
               if(_loc8_.@name)
               {
                  _loc4_ = _loc4_ + (" face=\'" + _loc8_.@name.toString() + "\'");
               }
            }
            if(_loc3_.@indent.toString() != "")
            {
               _loc5_ = _loc5_ + (" indent = \'" + _loc3_.@indent + "\'");
            }
            if(_loc3_.@blockIndent.toString() != "")
            {
               _loc5_ = _loc5_ + (" blockIndent = \'" + _loc3_.@blockInden + "\'");
            }
            if(_loc3_.@size.toString() != "")
            {
               _loc4_ = _loc4_ + (" size = \'" + _loc3_.@size + "\'");
            }
            if(_loc3_.@color.toString() != "")
            {
               _loc9_ = _loc3_.@color;
               if(_loc9_.substr(0,2) == "0x" || _loc9_.substr(0,2) == "0X")
               {
                  _loc9_ = "#" + _loc9_.substr(2);
               }
               _loc4_ = _loc4_ + (" color = \'" + _loc9_ + "\'");
            }
            if(_loc3_.@kerning.toString() != "")
            {
               _loc4_ = _loc4_ + (" kerning =\'" + _loc3_.@kerning + "\'");
            }
            if(_loc3_.@letterSpacing.toString() != "")
            {
               _loc4_ = _loc4_ + (" letterSpacing =\'" + _loc3_.@letterSpacing + "\'");
            }
            if(_loc3_.@leading.toString() != "")
            {
               _loc5_ = _loc5_ + (" leading = \'" + _loc3_.@leading + "\'");
            }
            ...
            if(_loc5_.length > 0)
            {
               _loc2_.leftTag = _loc2_.leftTag + ("<textformat " + _loc5_ + " >");
               _loc2_.rightTag = _loc2_.rightTag + "</textformat>";
            }
            if(_loc4_.length > 0)
            {
               _loc2_.leftTag = _loc2_.leftTag + ("<font " + _loc4_ + " >");
               _loc2_.rightTag = _loc2_.rightTag + "</font>";
            }
         }
         return _loc2_;
      }
