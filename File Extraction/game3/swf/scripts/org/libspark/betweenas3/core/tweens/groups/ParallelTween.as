   ...
   public class ParallelTween extends AbstractTween implements IITweenGroup
   {
      ...
      public function ParallelTween(param1:Array, param2:ITicker, param3:Number)
      {
         ...
         if(_loc4_ > 0)
         {
            ...
            _duration = this._a.duration > _duration ? Number(this._a.duration) : _duration;
            if(_loc4_ > 1)
            {
               this._b = param1[1] as IITween;
               _duration = this._b.duration > _duration ? Number(this._b.duration) : _duration;
               if(_loc4_ > 2)
               {
                  this._c = param1[2] as IITween;
                  _duration = this._c.duration > _duration ? Number(this._c.duration) : _duration;
                  if(_loc4_ > 3)
                  {
                     this._d = param1[3] as IITween;
                     _duration = this._d.duration > _duration ? Number(this._d.duration) : _duration;
                     if(_loc4_ > 4)
                     {
                        ...
                        while(_loc5_ < _loc4_)
                        {
                           ...
                           _duration = _loc6_.duration > _duration ? Number(_loc6_.duration) : _duration;
                           _loc5_++;
                        }
                     }
                  }
